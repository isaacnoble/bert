...
def build_examples(documents, max_seq_length):
    ...
    for document in documents:
        for context_end in range(1, len(document)-1):
            ...
            context_tokens = [
                token for line in document[context_start:context_end] for token in line]

            if len(context_tokens) > max_seq_length - 2:
                raise ValueError('context tokens ({}) exceed max seq length ({}) with document range {}-{}'.format(
                    len(context_tokens), max_seq_length - 2, context_start, context_end))
            ...
            real_tokens = [token for line in document[context_end:real_end]
                           for token in line]
            if len(real_tokens) > max_seq_length - 2:
                raise ValueError('real tokens ({}) exceed max seq length ({}) with document range {}-{}'.format(
                    len(real_tokens), max_seq_length - 2, context_end, real_end))
            examples.append(
...
def write_instance_to_example_files(generator_fn, output_files, hidden_size, splits=100):
    ...
    for (context_out, real_out, context_features, real_features) in generator_fn():
        ...
        features["context_embeddings"] = create_float_feature(
            context_embeddings)
        features["context_transformer"] = create_float_feature(
            context_transformer)
...
def main(_):
    ...
    write_instance_to_example_files(
        generator_fn, [FLAGS.output_file], bert_config.hidden_size)
